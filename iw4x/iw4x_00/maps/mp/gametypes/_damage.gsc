...
Callback_PlayerDamage_internal( eInflictor, eAttacker, victim, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	// FRIENDLYFIRE:
	//
	// Start with the attacker's current team. This is the normal case and is
	// sufficient for damage that reaches us at the time it is fired.
	//
	// There is a wrinkle with projectiles. A grenade, for example, may be in
	// flight long enough for its owner to switch teams or go to spectators.
	// Looking at the player at this point would then give us the new team, not
	// the team the grenade was thrown from. _weapons.gsc deals with this by
	// recording the team on the projectile when it is thrown, so below we can
	// use that value to recognize the old team as friendly too.
	//
	// Note that we cannot simply take the recorded team whenever one exists.
	// Consider a charge that was placed by one player and then shot by another.
	// The engine makes the player who shot it the attacker, but the charge still
	// has the team of the player who placed it. In that case the recorded team
	// says something about the charge, not about our attacker. So we only treat
	// it as the attacker's old team once we know the projectile still belongs
	// to that attacker.
	//
	// And if there is no attacker then leave the team undefined. The code below
	// can then handle such damage in the usual way.
	//
	attackerTeam = undefined;
	if ( isDefined( eAttacker ) )
		attackerTeam = eAttacker.team;

	firedFromTeam = undefined;
	if ( isDefined( eInflictor ) && isDefined( eInflictor.team ) && ( !isDefined( eInflictor.owner ) || ( eInflictor.owner == eAttacker ) ) )
		firedFromTeam = eInflictor.team;

	attackerIsHittingTeammate = level.teamBased && isDefined( eAttacker ) && ( victim != eAttacker ) &&
		( ( isDefined( attackerTeam ) && ( victim.pers[ "team" ] == attackerTeam ) ) || ( isDefined( firedFromTeam ) && ( victim.pers[ "team" ] == firedFromTeam ) ) );

	// FRIENDLYFIRE:
	//
	// There is one special case with owned entities. The entity that delivered
	// the damage can belong to the victim and, since these entities carry their
	// owner's team, the test above will have classified the hit as friendly
	// fire.
	//
	// That is not what we want for the owner's own explosive. A player is not
	// their own teammate, so let this damage continue through the normal damage
	// path.
	//
	// Note that NPC attackers are kept out of this case. Vehicles, turrets and
	// similar entities have separate handling below where damage from the
	// victim's own killstreak is turned into a suicide.
	//
	if ( attackerIsHittingTeammate && !attackerIsNPC && !isPlayer( eAttacker ) && isDefined( eAttacker.owner ) && ( eAttacker.owner == victim ) )
		attackerIsHittingTeammate = false;
